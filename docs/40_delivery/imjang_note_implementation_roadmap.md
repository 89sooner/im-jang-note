...
이 문서는 승인된 요구사항과 아키텍처(`../30_technical_architecture/*`)를 구현 가능한 vertical slice와 milestone(REL-001~REL-006)으로 분해한다. 새 제품 범위를 추가하지 않으며, 잠긴 ID 사전만 사용한다. 위험 결정은 ADR로 연결한다(ADR-003~006/008 open).
...
- open ADR에 의존하는 slice는 해당 slice 진입 전 ADR 확정을 진입 기준으로 둔다.
...
- Product: ADR-008(수익화) open — 핵심 기능 비종속, 결정 지연 허용.
...
| --- | --- | --- | --- |
| Kakao 연동 방식 미확정(ADR-003) | 지도 성능 불확실 | REL-002 진입 전 확정 | 진입 기준화 |
| 국토부 캐싱 파라미터 미확정(ADR-004) | 신선도/쿼터 튜닝 보류 | REL-002 | 진입 기준화 |
| 충돌 정책 미확정(ADR-005), 작성자 단독 편집 | 공동 편집 불가 | REL-006 | 의도된 범위 축소(FR-NOTE-006) |
| 동의 범위 미확정(ADR-006) | 삭제 시 공유 데이터 처리 보류 | REL-003 진입 전 확정 | 진입 기준화 |
| 수익화 보류(ADR-008 open) | 결제/광고 인프라 미설계 | PRD 결정 후 | 명시적 non-goal(현재) |
